...
for smellname in smells.keys():
	...
	writer = csv.writer(file('example repository\\the rest to check\\%s.csv' %smellname,'wb'))
	count = 0
	total = 0
	for line in reader:
		...
		if smellname=='LongLambdaFunction':
			...
			if int(line[4])>m1[index1] and (int(line[5])>m2[index1] or int(line[6])>m3[index1]):
				if int(line[4])>m1[index2] and (int(line[5])>m2[index2] or int(line[6])>m3[index2]):
					continue
				else:
					count = count+1
			...
			if (int(line[4])>m1[index1] and int(line[6])>m3[index1]) or int(line[5])>m2[index1]:
				if (int(line[4])>m1[index2] and int(line[6])>m3[index2]) or int(line[5])>m2[index2]:
					continue
				else:
					count = count+1
					writer.writerow(line)
			...
			m1 = smells[smellname][0]
			m2 = smells[smellname][1]
			if int(line[4])>m1[index1] or int(line[5])>m2[index1]:
				if int(line[4])>m1[index2] or int(line[5])>m2[index2]:
					continue
				else:
					count = count+1
					writer.writerow(line)
			...
			m2 = smells[smellname][1]
			m3 = smells[smellname][2]
			if (len(line[4])>0 and int(line[4])>m1[index1]) or (len(line[6])>0 and int(line[5])>m2[index1] and int(line[6])>m3[index1]):
				if (len(line[4])>0 and int(line[4])>m1[index2]) or (len(line[6])>0 and int(line[5])>m2[index2] and int(line[6])>m3[index2]):
					continue
				else:
					count = count+1
					writer.writerow(line)
			...
			if int(line[4])>m[index1] and int(line[4])<=m[index2]:
				count = count+1
				writer.writerow(line)
	count_log.write("total:%d  " %total)
	count_log.write('rest:%d\r\n' %count)